...
addhl -group / regions -default code kickstart \
    ...
    double_string '"' (?:^|[^\\])(?:\\\\)*(") '' \
    single_string "'" (?:^|[^\\])(?:\\\\)*(') '' \
    packages '^\h*(%packages)\b' '^\h*(%end)\b' '' \
    shell '^\h*(%(?:pre|pre-install|post))\b' '^\h*(%end)\b' ''
...
addhl -group /kickstart/packages regex '\`\h*(%packages)\b' 1:type
addhl -group /kickstart/packages regex '^\h*%end\b' 0:type
addhl -group /kickstart/shell regex '\`\h*(%(?:pre-install|pre|post))\b' 0:type
